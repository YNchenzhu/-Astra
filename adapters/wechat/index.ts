...
// WeChat (unlike Telegram/Feishu) cannot EDIT a sent message, so every buffer
// flush becomes a NEW chat bubble. Streaming mid-reply therefore splits one
// answer into several bubbles (often mid-sentence). We instead buffer the whole
// turn and flush once on completion; only genuinely over-limit replies are
// split, and `splitMessage` does that at paragraph/sentence boundaries.
const WECHAT_FLUSH_INTERVAL_MS = 600_000 // effectively "only on complete" (stall safety only)
const WECHAT_FLUSH_CHAR_THRESHOLD = Number.MAX_SAFE_INTEGER // disable mid-stream char flush
...
function getBlockBuffer(chatId: string): MessageBuffer {
  ...
  if (!buffer) {
    buffer = new MessageBuffer(
      ...
      WECHAT_FLUSH_INTERVAL_MS,
      WECHAT_FLUSH_CHAR_THRESHOLD,
    )
