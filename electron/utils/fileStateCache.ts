/**
 * LRU-based file state cache for read deduplication.
 *
 * Tracks recently-read file content + mtime so `read_file` can return
 * a lightweight "file_unchanged" stub when the file hasn't been modified
 * since the last read, saving context tokens.
 */
...
/**
 * Path-normalizing wrapper around LRUCache.
 * All keys are normalized so `/foo/../bar` and `/bar` hit the same entry.
 */
export class FileStateCache {
  ...
  constructor(maxEntries: number, maxSizeBytes: number) {
    this.cache = new LRUCache<string, FileState>({
      ...
      sizeCalculation: (value: FileState) => Math.max(1, Buffer.byteLength(value.content)),
    })
  }
